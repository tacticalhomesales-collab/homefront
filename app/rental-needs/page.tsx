...
const PROPERTY_TYPES = [
  ...
  { label: "No preference", value: "no_pref_property" },
] as const;
...
export default function RentalNeedsPage() {
  ...
  const initialPropertyType = useMemo(() => sp.get("desired_property_type") || "", [sp]);
  const initialBedrooms = useMemo(() => sp.get("desired_bedrooms") || "", [sp]);
  ...
  const handleContinue = () => {
    ...
    q.set("desired_property_type", propertyType);
    q.set("desired_bedrooms", bedrooms);

    // Also store a combined summary for use in review cards
    const summaryParts = [] as string[];
    if (propertyType) summaryParts.push(propertyType);
    if (bedrooms) summaryParts.push(`${bedrooms}_bedrooms`);
    if (summaryParts.length) {
      q.set("rental_needs", summaryParts.join(","));
    }
