...
export default function ReferPage() {
  ...
  const handleContinue = () => {
    ...
    // Store PII in sessionStorage (NOT in URL params)
    ...
    // Only mission goes in URL params (non-PII)
    const q = new URLSearchParams();
    q.set("mission", mission);
    q.set("mode", "referral");

    setTimeout(() => router.push(`/refer-location?${q.toString()}`), 120);
  };
  ...
  return (
    <FlowLayout title="Refer a Friend" subtitle="Help someone find their perfect housing solution.">
      <div className="relative z-50 flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-3 text-left">
          ...
          <label className="flex flex-col">
            <span className="text-xs font-bold text-white/70 uppercase tracking-wide mb-1.5">
              Friend's Email <span className="text-white/40">(Optional)</span>
            </span>
            <input
              type="email"
        ...
        <div className="flex flex-col gap-2 text-left mt-2">
          <span className="text-xs font-bold text-white/70 uppercase tracking-wide">
            What is your friend looking to do?
          </span>
