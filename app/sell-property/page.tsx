...
export default function SellPropertyPage() {
  ...
  const onSubmitLocation = () => {
    if (!location.trim() || pressed) return;
    setPressed(true);
    setTimeout(() => {
      setPressed(false);
      setStep(2);
  ...
  const onPickPropertyType = (value: string) => {
    if (pressed) return;
  ...
  const onPickOccupied = (value: string) => {
    if (pressed) return;
    setActiveLabel(value);
    setPressed(true);
    setOccupied(value);
    ...
    q.set("property_location", location);
    q.set("property_type", propertyType);
    q.set("property_occupied", value);

    const href = `/sell-timeline?${q.toString()}`;
