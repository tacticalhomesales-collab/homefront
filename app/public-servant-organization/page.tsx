...
export default function PublicServantOrganizationPage() {
  const router = useRouter();
  const sp = useSearchParams();

  const initialOrg = useMemo(() => sp.get("volunteer_org") || "", [sp]);
  const initialRole = useMemo(() => sp.get("volunteer_role") || "", [sp]);
  ...
  const goNext = () => {
    ...
    setTimeout(() => {
      ...
      q.set("volunteer_org", orgClean);
      q.set("volunteer_role", roleClean);
      ...
      <div className="w-full max-w-md mx-auto">
        ...
        <div className="mt-2 relative z-50 space-y-3">
          <input
            ...
            inputMode="text"
            placeholder="Organization (e.g., Nonprofit, Community Group)"
          ...
          <input
            ...
            inputMode="text"
            placeholder="Your role (e.g., Volunteer, Coordinator)"
            className="w-full rounded-2xl px-4 py-2.5 border border-white/15 bg-white/10 text-white text-[15px] font-extrabold placeholder:text-white/40 focus:outline-none focus-visible:ring-4 focus-visible:ring-[#ff385c]/30"
          />
        </div>
