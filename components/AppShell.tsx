...
export default function AppShell({ children, footer }: AppShellProps) {
  ...
  // Apply fit-to-viewport style to all pages (no fixed-height locking)
  const isShareRoute = pathname.startsWith("/share");
  return (
    <main
      className={[
        "w-full bg-[#0b0f14] flex flex-col items-center text-white px-4 overflow-x-hidden",
        "min-h-[100dvh]"
      ].join(" ")}
    >
      <div
        className={[
          "w-full max-w-[520px] flex flex-col mx-auto relative",
          "min-h-[100dvh]"
        ].join(" ")}
      >
        ...
        <header className="pt-2 pb-2 flex flex-col items-center justify-center relative">
          {pathname === "/" ? (
            <>
              <button
              ...
              <p className="mt-1 text-[10px] text-white/60 text-center">
                Tap the badge to share HomeFront.
              </p>
            </>
          ) : (
            <img
